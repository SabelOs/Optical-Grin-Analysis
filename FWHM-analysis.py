...
greens = plt.cm.Greens(np.linspace(0.4, 1, len(selected_lenses)))
reds = plt.cm.Reds(np.linspace(0.4, 1, len(selected_lenses)))
...
ax.legend(unique.values(), unique.keys(), loc="lower right", fontsize=12)

# -----------------------------
# INSET (ALL LENSES, FIXED BARS)
# -----------------------------
axins = inset_axes(ax, width="40%", height="30%", loc="upper left")
axins.yaxis.set_label_position("right")
axins.yaxis.tick_right()
axins.set_ylabel("$f_{calc}$ / mm", fontsize=12)
axins.tick_params(axis="both",labelsize=12)

offset = 1.5      # spacing between lens groups
bar_width = 0.6   # width of each bar

x_positions = np.arange(len(lens_names)) * offset

for i, lens in enumerate(lens_names):
    row_green = df_fit[(df_fit["measurement"] == lens) &
                       (df_fit["light_source"] == "green-He-Ne")]

    row_red = df_fit[(df_fit["measurement"] == lens) &
...
for source, cmap in zip(["green-He-Ne", "red-He-Ne"],
                        [plt.cm.Greens, plt.cm.Reds]):

    sub = df[(df["measurement"].str.contains("pinhole-200")) &
             (df["light_source"] == source)]
...
for source, color in zip(["green-He-Ne", "red-He-Ne"], ["green", "red"]):
    ...
    print(f"{source} divergence angle: {phi_deg:.4f}°")
    ...
    plt.plot(
        ...
        color=color,
        linewidth=2,
        label=f"{source.split('-')[0]} fit (φ = {phi_deg:.3f}°)"
    )
...
plt.ylabel("FWHM (µm)", fontsize=14)
